...
# Function for setting up the other grid values 1 4
# 1 4
#3 3
#2 3
def set_values():
...
def neighbours(r, col):
    ...
    if [r,col] not in vis:
        ...
        if numbers[r][col] == 0:
            ...
            if col < n-1: #right
                neighbours(r, col+1)    
            if r > 0 and col > 0: #top-left
                neighbours(r-1, col-1)
            if r > 0 and col < n-1: #top-right
                neighbours(r-1, col+1)
            if r < n-1 and col > 0: # bottom-left
                neighbours(r+1, col-1)
            if r < n-1 and col < n-1: #bottom-right
                neighbours(r+1, col+1)  
